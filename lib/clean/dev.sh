...
clean_github_cli_cache() {
    ...
    case "$_MOLE_GITHUB_CLI_CLEAR_REASON" in
        ...
        "process state unknown" | "cache path changed" | "owner cleanup failed")
            echo -e "  ${GRAY}${ICON_WARNING}${NC} GitHub CLI 缓存 · 已停止（${_MOLE_GITHUB_CLI_CLEAR_REASON}）"
            note_activity
            ;;
        *)
            echo -e "  ${GRAY}${ICON_WARNING}${NC} GitHub CLI 缓存 · 已停止（owner cleanup failed）"
            note_activity
            ;;
    esac
    return 0
...
clean_guarded_dev_cache_root() {
    ...
    if ! physical_root=$(guarded_dev_cache_root_physical_path "$container_root" "$cache_root"); then
        echo -e "  ${GRAY}${ICON_WARNING}${NC} ${display_name} · 已停止（cache path unsafe）"
        note_activity
        return 0
    fi
...
clean_go_cache_root() {
    ...
    # root in place.
    if [[ "$cache_kind" == "GOMODCACHE" && -L "$cache_root" ]]; then
        echo -e "  ${GRAY}${ICON_WARNING}${NC} ${display_name} · 已停止（symlinked module root）"
        note_activity
        return 0
    fi
    ...
    if [[ $process_state -eq 0 ]]; then
        ...
        echo -e "  ${GRAY}${ICON_WARNING}${NC} ${display_name} · 已停止（process state unknown）"
        note_activity
        return 0
    fi
    ...
    if [[ "$_MOLE_GO_CACHE_BOUND_REASON" == "Go started" ]]; then
        ...
        echo -e "  ${GRAY}${ICON_WARNING}${NC} ${display_name} · 已停止（owner cleanup failed）"
        note_activity
    fi
    return 0
}
...
clean_rust_dependency_cache_root() {
    ...
    if ! physical_root=$(rust_cache_root_physical_path "$cargo_home" "$cache_root"); then
        echo -e "  ${GRAY}${ICON_WARNING}${NC} ${display_name} · 已停止（cache path leaves CARGO_HOME）"
        note_activity
        return 0
    fi
...
clean_dev_rust() {
    ...
    if mole_cleanup_targets_exist \
        ...
        if [[ $rust_state -eq 0 ]]; then
            ...
            echo -e "  ${GRAY}${ICON_WARNING}${NC} Rust 依赖缓存 · 已停止（process state unknown）"
            note_activity
        fi
    fi
...
clean_xcode_documentation_cache() {
    ...
    if [[ $process_state -ne 1 ]]; then
        ...
        if [[ $process_state -eq 2 ]]; then
            echo -e "  ${GRAY}${ICON_WARNING}${NC} Xcode 文档缓存 · 已跳过（process state unknown）"
            note_activity
        else
...
clean_xcode_xctest_devices() {
    ...
    if [[ $process_state -ne 1 ]]; then
        if [[ $process_state -eq 2 ]]; then
            echo -e "  ${GRAY}${ICON_WARNING}${NC} Xcode XCTestDevices · 已跳过（process state unknown）"
            note_activity
        else
            mole_defer_cleanup_family "Xcode"
        fi
...
clean_xcode_system_coresimulator_caches() {
    ...
    if [[ $process_state -ne 1 ]]; then
        if [[ $process_state -eq 2 ]]; then
            echo -e "  ${GRAY}${ICON_WARNING}${NC} Xcode 模拟器系统缓存 · 已跳过（process state unknown）"
            note_activity
        else
            mole_defer_cleanup_family "Simulator"
        fi
...
clean_xcode_device_support() {
    ...
    if [[ ${#preflight_dirs[@]} -gt $keep_count ]]; then
        ...
        for candidate in "${preflight_dirs[@]}"; do
            if ! preflight_mtime=$(stat -f%m "$candidate" 2> /dev/null) || [[ ! "$preflight_mtime" =~ ^[0-9]+$ ]]; then
                echo -e "  ${GRAY}${ICON_WARNING}${NC} ${display_name} · 已跳过（metadata unavailable）"
                note_activity
                return 0
            fi
    ...
    if [[ $process_state -ne 1 ]]; then
        if [[ $process_state -eq 2 ]]; then
            echo -e "  ${GRAY}${ICON_WARNING}${NC} ${display_name} · 已跳过（process state unknown）"
            note_activity
        else
            mole_defer_cleanup_family "Xcode"
        fi
...
clean_xcode_simulator_runtime_volumes() {
    ...
    if [[ "$DRY_RUN" == "true" ]]; then
        ...
        if [[ "$total_entries" -gt "$shown" ]]; then
            ...
            echo -e "    ${GRAY}${ICON_LIST}${NC} ... 另有 ${remaining} 个运行时卷条目"
        fi
        note_activity
        if [[ -n "$dry_stop_reason" ]]; then
    ...
    if [[ $removed_count -gt 0 ]]; then
        ...
        if [[ $skipped_protected -gt 0 ]]; then
            echo -e "  ${line_color}${ICON_SUCCESS}${NC} Xcode 运行时卷 · 已移除 ${removed_count} 个（${line_color}${removed_human}${NC}），已跳过 ${skipped_protected} 个受保护"
        else
            echo -e "  ${line_color}${ICON_SUCCESS}${NC} Xcode 运行时卷 · 已移除 ${removed_count} 个（${line_color}${removed_human}${NC}）"
        fi
        if [[ $failed_count -gt 0 ]]; then
            echo -e "  ${GRAY}${ICON_WARNING}${NC} Xcode 运行时卷 · 无法移除 ${failed_count} 个条目"
        fi
        note_activity
    else
        if [[ $failed_count -gt 0 ]]; then
            echo -e "  ${GRAY}${ICON_WARNING}${NC} Xcode 运行时卷 · 无法移除 ${failed_count} 个条目"
            if [[ $skipped_protected -gt 0 ]]; then
                echo -e "  ${YELLOW}${ICON_WARNING}${NC} Xcode 运行时卷 · 已跳过 ${skipped_protected} 个受保护"
            fi
        elif [[ $skipped_protected -gt 0 ]]; then
            echo -e "  ${YELLOW}${ICON_WARNING}${NC} Xcode 运行时卷 · 已跳过 ${skipped_protected} 个受保护，未移除"
        fi
        ...
        if [[ "$_MOLE_SIMCTL_RESOLUTION_STATUS" == "ambiguous" ]]; then
            ...
            if [[ "$simctl_available" == "true" ]]; then
                ...
                if [[ $unavailable_list_exit_code -ne 0 ]]; then
                    echo -e "  ${GRAY}${ICON_WARNING}${NC} Xcode 不可用的模拟器 · simctl list 失败（exit=${unavailable_list_exit_code}）"
...
clean_dev_jvm() {
    ...
    if mole_cleanup_targets_exist \
        ...
        if [[ $gradle_state -eq 0 ]]; then
            ...
            echo -e "  ${GRAY}${ICON_WARNING}${NC} Gradle targets · 已跳过（process state unknown）"
            note_activity
        fi
    fi
}
...
clean_claude_desktop_bundled_versions() {
    ...
    if [[ -z "$sdk_version" ]]; then
        note_activity
        echo -e "  ${GRAY}${ICON_WARNING}${NC} Claude Desktop bundled Claude Code · 已跳过（active version unknown）"
        return 0
    fi
    ...
    for spec in "${desktop_specs[@]}"; do
        ...
        if [[ -L "$active_entry" || (! -f "$active_entry" && ! -d "$active_entry") ]]; then
            note_activity
            echo -e "  ${GRAY}${ICON_WARNING}${NC} $label · 已跳过（active version unknown）"
            return 0
        fi
    done
    ...
    if [[ $process_state -ne 1 ]]; then
        if [[ $process_state -eq 2 ]]; then
            echo -e "  ${GRAY}${ICON_WARNING}${NC} Claude Desktop bundled Claude Code · 已跳过（process state unknown）"
            note_activity
        else
            mole_defer_cleanup_family "Claude Desktop"
        fi
...
clean_codex_desktop_caches() {
    ...
    if [[ $process_state -ne 1 ]]; then
        if [[ $process_state -eq 2 ]]; then
            echo -e "  ${GRAY}${ICON_WARNING}${NC} Codex Desktop 缓存 · 已跳过（process state unknown）"
            note_activity
        else
...
clean_codex_desktop_staging() {
    ...
    if [[ $process_state -ne 1 ]]; then
        if [[ $process_state -eq 2 ]]; then
            echo -e "  ${GRAY}${ICON_WARNING}${NC} Codex Desktop 更新暂存 · 已跳过（process state unknown）"
            note_activity
        else
            mole_defer_cleanup_family "Codex"
        fi
    ...
    if [[ $updater_state -ne 1 ]]; then
        if [[ $updater_state -eq 2 ]]; then
            echo -e "  ${GRAY}${ICON_WARNING}${NC} Codex Desktop 更新暂存 · 已跳过（updater state unknown）"
            note_activity
        else
            mole_defer_cleanup_family "Codex"
        fi
    ...
    if [[ "$open_file_state" -eq 2 ]]; then
        echo -e "  ${GRAY}${ICON_WARNING}${NC} Codex Desktop 更新暂存 · 已跳过（open-file check unavailable）"
        note_activity
        return 0
    fi
...
clean_codex_runtimes() {
    ...
    if [[ $process_state -ne 1 ]]; then
        if [[ "$has_stale_runtime" != "true" ]]; then
            ...
            echo -e "  ${GRAY}${ICON_WARNING}${NC} Codex 运行时 · 已跳过（process state unknown）"
            note_activity
        else
            mole_defer_cleanup_family "Codex"
        fi
...
clean_codex_marketplace_staging() {
    ...
    if [[ $process_state -ne 1 ]]; then
        if [[ $process_state -eq 2 ]]; then
            echo -e "  ${GRAY}${ICON_WARNING}${NC} Codex marketplace 暂存 · 已跳过（process state unknown）"
            note_activity
        else
            mole_defer_cleanup_family "Codex"
        fi
    ...
    for stale_entry in "${stale_entries[@]}"; do
        ...
        if [[ "$open_file_state" -eq 2 ]]; then
            echo -e "  ${GRAY}${ICON_WARNING}${NC} Codex marketplace 暂存 · 已跳过（open-file check unavailable）"
            note_activity
            return 0
        fi
        ...
        if [[ $guarded_rc -eq 75 ]]; then
            case "${_MOLE_CLEAN_GUARD_REASON:-}" in
                "process state unknown" | "open-file check unavailable")
                    echo -e "  ${GRAY}${ICON_WARNING}${NC} Codex marketplace 暂存 · 已停止（${_MOLE_CLEAN_GUARD_REASON}）"
                    note_activity
                    ;;
                "staging entry changed")
...
clean_antigravity_caches() {
    ...
    if [[ $process_state -ne 1 ]]; then
        if [[ $process_state -eq 2 ]]; then
            echo -e "  ${GRAY}${ICON_WARNING}${NC} Antigravity/Gemini 缓存 · 已跳过（process state unknown）"
            note_activity
        else
...
clean_chrome_devtools_mcp_caches() {
    ...
    if [[ $process_state -ne 1 ]]; then
        if [[ $process_state -eq 2 ]]; then
            echo -e "  ${GRAY}${ICON_WARNING}${NC} Chrome DevTools MCP 缓存 · 已跳过（process state unknown）"
            note_activity
        else
