...
This document describes the security-relevant behavior of the current `main` branch, updated for V1.51.0 on 2026-08-16. It is intended as a public description of Mole's safety boundaries, destructive-operation controls, release integrity signals, and known limitations.
...
- live user-cache refusal while an owner process runs, an unreadable process table denying rather than allowing, and SQLite family members held by an open handle or a WAL `-shm` companion (`tests/core_safe_functions.bats`, `tests/clean_core.bats`)
